---
title: App reference
slug: create-apps-create-apps-app-reference
section: Create-Apps
---
...
| `disk`             | `integer` or `null`                                 |          | Yes              | The size of the disk space for the app in [MB](../../glossary#mb). Minimum value is `128`. Defaults to `null`, meaning no disk is available. See [note on available space](#available-disk-space) |
...
in reality **all** Web PaaS mounts are [Network Storage](../add-services-network-storage) mounts.
...
If your project _does_ required isolated, instance-specific data, you can leverage the [`/tmp` directory](../create-apps-hooks/hooks-comparison#build-hook), the [`pre_start`](#web-commands) web command, and symlinks to acheive it.
...
Remember that the `/tmp` has a [maximum allocation of 4 GB](../create-apps-troubleshoot-disks#no-space-left-on-device). 
If your project requires more disk, [contact Support](../learn-overview/get-support).
...
Relying on Web PaaS hooks for such tasks can prove difficult.
